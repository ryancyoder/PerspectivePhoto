...
export function LightPropertiesPanel() {
  ...
  const sliders: { label: string; key: string; min: number; max: number; step: number; value: number }[] = [
    { label: 'Radius', key: 'radius', min: 20, max: 300, step: 5, value: light.radius },
    { label: 'Intensity', key: 'intensity', min: 0, max: 1, step: 0.05, value: light.intensity },
    { label: 'Spread X', key: 'spreadX', min: 0.3, max: 2.5, step: 0.1, value: light.spreadX },
    { label: 'Spread Y', key: 'spreadY', min: 0.3, max: 2.5, step: 0.1, value: light.spreadY },
        ...
        <div className="flex gap-1 shrink-0">
          {(['uplight', 'path', 'spotlight'] as LightPreset[]).map((preset) => (
            <button
              key={preset}
              onClick={() => {
                const defaults: Record<string, { spreadX: number; spreadY: number; radius: number }> = {
                  uplight:   { spreadX: 0.6, spreadY: 1.8, radius: 120 },
                  path:      { spreadX: 1.0, spreadY: 1.0, radius: 80 },
                  spotlight: { spreadX: 0.7, spreadY: 1.2, radius: 150 },
                };
                const d = defaults[preset];
                updateLight(light.id, { type: preset, spreadX: d.spreadX, spreadY: d.spreadY, radius: d.radius });
              }}
